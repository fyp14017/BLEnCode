...
import hku.fyp14017.blencode.transfers.ProjectUploadService;
...
public class UploadProjectDialog extends DialogFragment {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		View dialogView = LayoutInflater.from(getActivity()).inflate(hku.fyp14017.blencode.R.layout.dialog_upload_project, null);

		projectRename = (TextView) dialogView.findViewById(hku.fyp14017.blencode.R.id.tv_project_rename);
		projectDescriptionField = (EditText) dialogView.findViewById(hku.fyp14017.blencode.R.id.project_description_upload);
		projectUploadName = (EditText) dialogView.findViewById(hku.fyp14017.blencode.R.id.project_upload_name);
		sizeOfProject = (TextView) dialogView.findViewById(hku.fyp14017.blencode.R.id.dialog_upload_size_of_project);
		...
		Dialog dialog = new AlertDialog.Builder(getActivity()).setView(dialogView)
				...
				.setPositiveButton(hku.fyp14017.blencode.R.string.upload_button, new DialogInterface.OnClickListener() {
					@Override
	...
	private void handleUploadButtonClick() {
		...
		if (uploadName.equals(getString(hku.fyp14017.blencode.R.string.default_project_name))) {
			Utils.showErrorDialog(getActivity(), hku.fyp14017.blencode.R.string.error_upload_project_with_default_name);
			return;
		}

		Context context = getActivity().getApplicationContext();
		if (Utils.isStandardProject(projectManager.getCurrentProject(), context)) {
			Utils.showErrorDialog(getActivity(), hku.fyp14017.blencode.R.string.error_upload_default_project);
			return;
		}
