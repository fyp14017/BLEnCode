	...
	class ScreenshotLoader implements Runnable {
		...
		public void run() {
			...
			if (!projectImageFile.exists() || ImageEditing.getImageDimensions(pathOfScreenshot)[0] < 0) {
				...
				int width = context.getResources().getDimensionPixelSize(hku.fyp14017.blencode.R.dimen.project_thumbnail_width);
				int height = context.getResources().getDimensionPixelSize(hku.fyp14017.blencode.R.dimen.project_thumbnail_height);
