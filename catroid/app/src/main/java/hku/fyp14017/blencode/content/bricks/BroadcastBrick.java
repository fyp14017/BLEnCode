...
public class BroadcastBrick extends BrickBaseType implements BroadcastMessage {
	...
	public View getView(final Context context, int brickId, BaseAdapter baseAdapter) {
		...
		final Spinner broadcastSpinner = (Spinner) view.findViewById(hku.fyp14017.blencode.R.id.brick_broadcast_spinner);
	...
	public View getPrototypeView(Context context) {
		...
		Spinner broadcastSpinner = (Spinner) prototypeView.findViewById(hku.fyp14017.blencode.R.id.brick_broadcast_spinner);
		broadcastSpinner.setFocusableInTouchMode(false);
		broadcastSpinner.setFocusable(false);
	...
	public View getViewWithAlpha(int alphaValue) {
		...
		if (view != null) {
			...
			TextView textBroadcastLabel = (TextView) view.findViewById(hku.fyp14017.blencode.R.id.brick_broadcast_label);
			textBroadcastLabel.setTextColor(textBroadcastLabel.getTextColors().withAlpha(alphaValue));
			Spinner broadcastSpinner = (Spinner) view.findViewById(hku.fyp14017.blencode.R.id.brick_broadcast_spinner);
	...
	protected void showNewMessageDialog(final Spinner spinner) {
		...
		BrickTextDialog editDialog = new BrickTextDialog() {
			...
			protected boolean handleOkButton() {
				...
				if (newMessage.isEmpty() || newMessage.equals(context.getString(hku.fyp14017.blencode.R.string.new_broadcast_message))) {
					dismiss();
					return false;
				}
