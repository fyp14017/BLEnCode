...
import hku.fyp14017.blencode.CatroidApplication;
...
public class DroneInitializer implements DroneReadyReceiverDelegate, DroneConnectionChangeReceiverDelegate,
	...
	public void initialiseDrone() {
		if (!CatroidApplication.OS_ARCH.startsWith("arm")) {
			...
			showUncancelableErrorDialog(prestageStageActivity,
					prestageStageActivity.getString(hku.fyp14017.blencode.R.string.error_drone_wrong_platform_title),
					prestageStageActivity.getString(hku.fyp14017.blencode.R.string.error_drone_wrong_platform));
			return;
		}
	...
	public void onDroneReady() {
		...
		if (droneBatteryCharge < DRONE_BATTERY_THRESHOLD) {
			String dialogTitle = String.format(prestageStageActivity.getString(hku.fyp14017.blencode.R.string.error_drone_low_battery_title),
					droneBatteryCharge);
			showUncancelableErrorDialog(prestageStageActivity, dialogTitle,
					prestageStageActivity.getString(hku.fyp14017.blencode.R.string.error_drone_low_battery));
			return;
		}
	...
	public void onDroneAvailabilityChanged(boolean isDroneOnNetwork) {
		...
		if (isDroneOnNetwork) {
			...
			showUncancelableErrorDialog(prestageStageActivity,
					prestageStageActivity.getString(hku.fyp14017.blencode.R.string.error_no_drone_connected_title),
					prestageStageActivity.getString(hku.fyp14017.blencode.R.string.error_no_drone_connected));
		}
	}
