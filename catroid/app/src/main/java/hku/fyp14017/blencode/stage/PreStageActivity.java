...
import hku.fyp14017.blencode.ble.RssiScanner;
import hku.fyp14017.blencode.ble.SensorInfo;
import hku.fyp14017.blencode.ble.SensorTag;
import hku.fyp14017.blencode.bluetooth.BluetoothManager;
import hku.fyp14017.blencode.bluetooth.DeviceListActivity;
...
import hku.fyp14017.blencode.content.actions.MonitorSensorAction;
...
import hku.fyp14017.blencode.legonxt.LegoNXTBtCommunicator;
...
public class PreStageActivity extends BaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(hku.fyp14017.blencode.R.layout.activity_prestage);
        SensorTagCounter = 0;
        CardCounter = 0;
        ...
        if ((requiredResources & Brick.BLUETOOTH_BLE_SENSORS) > 0) {
            ...
            if (bluetoothState == BluetoothManager.BLUETOOTH_NOT_SUPPORTED) {
                ...
                Toast.makeText(PreStageActivity.this, hku.fyp14017.blencode.R.string.notification_blueth_err, Toast.LENGTH_LONG).show();
                resourceFailed();
            } else {
		...
		if ((requiredResources & Brick.CAMERA_LED ) > 0) {
			if (!CameraManager.getInstance().isFacingBack()) {
				...
				builder.setMessage(getString(hku.fyp14017.blencode.R.string.led_and_front_camera_warning)).setCancelable(false)
						.setPositiveButton(getString(hku.fyp14017.blencode.R.string.ok), new DialogInterface.OnClickListener() {
							@Override
		...
		if ((requiredResources & Brick.VIBRATOR) > 0) {
			...
			if (vibrator != null) {
				...
				Toast.makeText(PreStageActivity.this, hku.fyp14017.blencode.R.string.no_vibrator_available, Toast.LENGTH_LONG).show();
				resourceFailed();
			}
		}
	...
	protected boolean hasFlash() {
		...
		try {
			...
			Log.e(getString(hku.fyp14017.blencode.R.string.app_name), "failed to open Camera", exception);
		}
	...
	private void startBluetoothCommunication(boolean autoConnect) {
		if(connectingProgressDialogFlag) {
            connectingProgressDialog = ProgressDialog.show(this, "",
                    getResources().getString(hku.fyp14017.blencode.R.string.connecting_please_wait), true);
		...
		switch (requestCode) {
			case REQUEST_ENABLE_BLUETOOTH:
				switch (resultCode) {
					...
					case Activity.RESULT_CANCELED:
						Toast.makeText(PreStageActivity.this, hku.fyp14017.blencode.R.string.notification_blueth_err, Toast.LENGTH_LONG)
								.show();
						resourceFailed();
						break;
				}
			...
			case REQUEST_CONNECT_DEVICE:
				switch (resultCode) {
					...
					case Activity.RESULT_CANCELED:
						connectingProgressDialog.dismiss();
						Toast.makeText(PreStageActivity.this, hku.fyp14017.blencode.R.string.bt_connection_failed, Toast.LENGTH_LONG).show();
						resourceFailed();
						break;
				}
				break;
			...
			case REQUEST_TEXT_TO_SPEECH:
				if (resultCode == TextToSpeech.Engine.CHECK_VOICE_DATA_PASS) {
					...
					builder.setMessage(hku.fyp14017.blencode.R.string.text_to_speech_engine_not_installed).setCancelable(false)
		...
		public void handleMessage(Message myMessage) {
			...
			switch (myMessage.getData().getInt("message")) {
				...
				case LegoNXTBtCommunicator.STATE_CONNECTERROR:
					Toast.makeText(PreStageActivity.this, hku.fyp14017.blencode.R.string.bt_connection_failed, Toast.LENGTH_SHORT).show();
					connectingProgressDialog.dismiss();
					legoNXT.destroyCommunicator();
					legoNXT = null;
					if (autoConnect) {
	...
	private void ledInitialize() {
		if ( hasFlash() ) {
			...
			Toast.makeText(PreStageActivity.this, hku.fyp14017.blencode.R.string.no_flash_led_available, Toast.LENGTH_LONG).show();
			resourceFailed();
		}
	}
