...
public class NewVariableDialog extends SherlockDialogFragment {
	...
	public Dialog onCreateDialog(Bundle bundle) {
		final View dialogView = LayoutInflater.from(getActivity()).inflate(
				hku.fyp14017.blencode.R.layout.dialog_formula_editor_variable_name, null);

		final Dialog dialogNewVariable = new AlertDialog.Builder(getActivity()).setView(dialogView)
				.setTitle(hku.fyp14017.blencode.R.string.formula_editor_variable_dialog_title)
				.setNegativeButton(hku.fyp14017.blencode.R.string.cancel_button, new OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						dialog.cancel();
					}
				}).setPositiveButton(hku.fyp14017.blencode.R.string.ok, new OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
	...
	private void handleOkButton(View dialogView) {
		EditText variableNameEditText = (EditText) dialogView
				.findViewById(hku.fyp14017.blencode.R.id.dialog_formula_editor_variable_name_edit_text);
		RadioButton localVariable = (RadioButton) dialogView
				.findViewById(hku.fyp14017.blencode.R.id.dialog_formula_editor_variable_name_local_variable_radio_button);
		RadioButton globalVariable = (RadioButton) dialogView
				.findViewById(hku.fyp14017.blencode.R.id.dialog_formula_editor_variable_name_global_variable_radio_button);
		...
		if (globalVariable.isChecked()) {
			if (ProjectManager.getInstance().getCurrentProject().getUserVariables()
				...
				Toast.makeText(getActivity(), hku.fyp14017.blencode.R.string.formula_editor_existing_variable, Toast.LENGTH_LONG).show();
		...
		dialogEditText.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable editable) {
				...
				if (ProjectManager.getInstance().getCurrentProject().getUserVariables()
					...
					Toast.makeText(getActivity(), hku.fyp14017.blencode.R.string.formula_editor_existing_variable, Toast.LENGTH_SHORT).show();
